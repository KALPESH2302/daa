...
The printBoard function iterates through the entire 2D board, which has a size of n x n, resulting in a time complexity of O(n^2).
Space Complexity: O(1)
The function uses a constant amount of extra space for variables, regardless of the input size.

Overall Time Complexity: The worst-case time complexity is generally less than O(n^n) due to backtracking and pruning. In practice, the algorithm often finds solutions more quickly because it avoids exploring many invalid positions.

The overall space complexity can be summarized as follows:

Overall Space Complexity: O(n^2) in the worst case. This is mainly due to the 2D board array, which has a size of n x n, and the recursion stack that can go up to n levels deep. The space complexity is bounded by the size of the board
 */
